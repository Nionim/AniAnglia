...
inline void InlineJson::close_object(std::string& json_str) {
...
inline void InlineJson::close_array(std::string& json_str) {
    if (!json_str.empty() && json_str.back() == ',')
        json_str.back() = ']';
    else
        json_str += ']';
...
inline std::string InlineJson::escape_string(std::string_view str) {
    std::string out;
...
template<typename T>
void InlineJson::raw_append(std::string& json_str, T&& value) {
...
template<typename T>
void InlineJson::append(std::string& json_str, std::string_view key, T&& value) {
    json_str += '"';
    json_str += key;
    json_str += R"(":)";
...
template<typename Value, typename Serializator>
void InlineJson::append(std::string& json_str, std::string_view key, Value&& value, Serializator&& serializator) {
    json_str += '"';
    json_str += key;
    json_str += R"(":)";
...
template<typename T>
void InlineJson::append(std::string& json_str, T&& value) {
    raw_append(json_str, std::forward<T>(value));
    json_str += ",";
}

template<typename Value, typename Serializator>
void InlineJson::append(std::string& json_str, Value&& value, Serializator&& serializator) {
...
template<typename T>
    requires serializable_to_json_array<std::remove_cvref_t<T>>
std::string InlineJson::serialize(T&& arr) {
    if (arr.empty()) return "[]";
    std::string json_str;
    open_array(json_str);
    for (auto& val : arr) {
...
template<typename Array, typename Serializator>
    requires serializable_to_json_array<std::remove_cvref_t<Array>>
std::string InlineJson::serialize(Array&& arr, Serializator&& serializator) {
    if (arr.empty()) return "[]";
    std::string json_str;
    open_array(json_str);
    for (auto& val : arr) {
...
template<typename T>
    requires serializable_to_json_object<std::remove_cvref_t<T>>
std::string InlineJson::serialize(T&& map) {
    if (map.empty()) return "{}";
    std::string json_str;
...
public:
    ...
    template<> static JsonObject& strong_get(JsonObject& object, const std::string_view key);
    template<> static JsonArray& strong_get(JsonObject& object, const std::string_view key);
    ...
    template<> static JsonObject& get_if(JsonObject& object, const std::string_view key, PredicateFunc predicate);
    template<> static JsonArray& get_if(JsonObject& object, const std::string_view key, PredicateFunc predicate);
...
template<> inline JsonObject& ParseJson::strong_get(JsonObject& object, const std::string_view key) {
    return object[key].as_object();
}
template<> inline JsonArray& ParseJson::strong_get(JsonObject& object, const std::string_view key) {
    return object[key].as_array();
}

template<typename T>
T ParseJson::get_if(JsonObject& object, const std::string_view key, PredicateFunc predicate) {
...
template<> inline JsonObject& ParseJson::get_if(JsonObject& object, const std::string_view key, PredicateFunc predicate) {
    return predicate(object, key) ? object[key].as_object() : NULL_OBJECT;
}

template<> inline JsonArray& ParseJson::get_if(JsonObject& object, const std::string_view key, PredicateFunc predicate) {
